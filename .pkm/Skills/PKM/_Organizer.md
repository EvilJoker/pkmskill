...
在逐个分析文件之前，若存在 `.pkm/Skills/PKM/plugin/Skill.md`，则对 50_Raw 中每个文件执行插件预处理：

1. **读取插件注册表**：读取 `plugin/Skill.md`，获取插件列表（插件名、匹配条件、模版文件）。
...
**约定**：模版文件命名 `template_<内容类型>.md`，位于 `.pkm/Skills/PKM/plugin/` 下。新增插件时在 `plugin/Skill.md` 的插件列表中增加一行，并新增对应模版文件。
