...
**重要**：添加任务时，必须采用交互式流程，逐步询问用户并等待确认，不能一次性完成所有步骤。每个字段询问后必须暂停执行，等待用户输入和确认。

---

## 触发时机

- **添加任务**：用户执行 `/pkm todo <内容>` 时
- **更新任务**：用户执行 `/pkm todo update <id/name>` 时
- **完成任务**：用户执行 `/pkm todo ok <id/name>` 时
- **删除任务**：用户执行 `/pkm todo del <id/name>` 时
- **列出任务**：用户执行 `/pkm todo list` 时
...
## 操作 1：添加新任务 (`/pkm todo <内容>`)

**重要说明**：此操作是交互式流程，必须逐步询问用户并等待确认，不能一次性完成所有步骤。每个字段询问后必须暂停执行，等待用户输入。
...
象限说明：
- 1️⃣ 第一象限：重要且紧急（需要立即处理）
- 2️⃣ 第二象限：重要但不紧急（需要规划）
- 3️⃣ 第三象限：不重要但紧急（快速处理）
- 4️⃣ 第四象限：不重要且不紧急（可延后）
...
1. **简单快速**：添加任务应该快速完成，不阻塞用户
2. **信息完整**：鼓励用户补全任务信息，但不强制
3. **进展追踪**：每次更新都记录时间戳和进展内容
4. **价值沉淀**：完成任务时记录总结，便于后续回顾
5. **归档管理**：已完成任务自动归档，保持 todo.md 轻量
