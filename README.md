...
> 基于 **PARA + CODE** 方法论的 AI 驱动知识管理系统，让 AI 成为你的知识管家
...
- **30_Resources（资源）**：参考资料，`00_Inbox` 作为捕捉起点
...
- **📥 Capture（捕获）**：快速记录，零心理负担
- **📋 Organize（组织）**：AI 自动分类，告别手动整理
- **💎 Distill（蒸馏）**：提炼精华，生成结构化知识
- **✨ Express（表达）**：输出成果，知识归档回流

#### 🛡️ 双轨制（人机协作）

- **📜 Manual（人脑主权区）**：你验证过的真理，AI 只读不写
- **🤖 AI_Synthesized（AI 自由区）**：AI 自动生成草稿，可随时删除

---

## ✨ 核心特性

- 🎯 **统一命令入口**：通过 `/pkm` (Cursor) 或 `@PKM` (Claude Code) 一键管理知识库
...
| `/pkm addProject` | 创建新项目 | 快速创建项目目录（自动命名：时间戳_XXX） |
...
| `/pkm` | **智能全自动模式**（最常用） | 每天结束时执行，自动完成所有整理工作 |
...
执行流程：
1. ✅ 验证知识库结构（Verifier）
2. 📥 如果 Inbox 有文件 → 自动分类（Classifier）
3. 📚 如果某主题积累 10+ 碎片 → 自动蒸馏（Synthesizer）
4. 🔍 如果有新草稿 → 自动审计（Auditor）
5. 📦 如果有项目标记完成 → 自动归档（Archiver）
```
...
以下命令是 `/pkm` 智能模式的子流程，通常由 `/pkm` 自动调用。如需手动执行：

| 命令 | 功能 | 说明 |
| ---- | ---- | ---- |
| `/pkm classify` | 分类 Inbox 内容 | `/pkm` 会自动执行，无需单独调用 |
| `/pkm synthesize <主题>` | 蒸馏知识 | `/pkm` 会自动执行，如需指定主题可手动调用 |
| `/pkm audit <主题>` | 质量审计 | `/pkm` 会自动执行，如需指定主题可手动调用 |
...
- **PARA**：Tiago Forte 的项目组织方法（Projects, Areas, Resources, Archives）
