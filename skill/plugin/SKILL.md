...
与 `docs/ARCHITECTURE.md` 5.4 一致。本目录下的插件在 **Organizer 处理 50_Raw/** 时生效：在「逐个分析文件」之前，先根据内容类型匹配插件，命中则按对应模版整理内容，再继续类型判断与分类归位。
...
1. 若存在 `skill/plugin/SKILL.md`（相对于 PKM 根目录），则读取本注册表。
