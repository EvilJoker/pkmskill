...
- **编辑任务**：用户执行 `@pkm todo edit <id>` 时
- **完成任务**：用户执行 `@pkm todo done <id>` 时
- **删除任务**：用户执行 `@pkm todo delete <id>` 时
- **列出任务**：用户执行 `@pkm todo ls` 或 `@pkm todo ls --done` 时（需额外做进展核查与延期风险提示，促进及时处理）
...
2. `@pkm todo ls` → 列出所有任务
3. `@pkm todo ls --done` → 列出已完成任务
4. `@pkm todo edit <id>` → 编辑任务内容
5. `@pkm todo update <id>` → 更新任务进展
6. `@pkm todo done <id>` → 完成任务
7. `@pkm todo delete <id>` → 删除任务
...
💡 提示：
  ...
  - 使用 `@pkm todo done T-20260113-143000-001` 完成任务
  - 使用 `@pkm todo ls` 查看所有任务
```

---

## 操作 2：编辑任务 (`@pkm todo edit <id>`)

### 步骤 1：查找任务

在 `todo.md` 中查找指定任务（支持通过 ID 或名称关键词查找）。若匹配多个，列出让用户选择。

### 步骤 2：编辑内容

询问用户要修改的字段（想法/目标、计划、实现思路、关联项目等），或直接提供当前内容供用户修改。将修改写回 `30_Resources/todo.md` 中该任务条目。
...
## 操作 6：列出任务 (`@pkm todo ls`)

执行 `@pkm todo ls` 时，除按四象限展示任务列表外，**必须额外做进展核查与延期风险提示**：分析各任务进展，提醒用户延期风险，促进及时处理。
...
3. **输出提醒**：用简短段落或列表汇总「进展核查结论」与「延期/风险提醒」，并建议用户对高风险或长期停滞的任务使用 `@pkm todo update` 更新进展或 `@pkm todo done` 关闭/归档，以**及时处理**、避免堆积。
...
💡 提示：
  - 使用 `@pkm todo update <id>` 更新任务
  - 使用 `@pkm todo done <id>` 完成任务
  - 使用 `@pkm todo delete <id>` 删除任务
```
