...
- 🔄 **Workspace 同步检测**（仅在 OpenClaw 会话中）- 检测 workspace 中是否有未纳入 PKM 的项目，询问用户是否合并
...
```text
@pkm workspace ls           # 查看 workspace 同步状态
@pkm workspace add <项目>   # 将 PKM 项目同步到 workspace
@pkm workspace sync         # 手动触发全量同步检测
```
...
- `workspace <操作>` → `_Verifier` → `_WorkspaceManager`（工作区管理）
...
| `_WorkspaceManager` | 工作区管理器 | 支撑 | `@pkm workspace`，主流程中的同步检测 |
