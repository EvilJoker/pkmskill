...
| `@pkm task use <task_name>` | 使用 | 切换到指定任务，并加载任务区信息 |
...
- **元数据**（权威数据源，tasks.md 仅存索引）：任务 ID、标题、创建时间、4 象限、分类、优先级、标签。
